...
#hexbin chart
hexbin_chart = alt.Chart(combined_point_data).mark_rect().encode(
    x=alt.X('Volume:Q', bin=alt.Bin(maxbins=50), title='Volume (vehicles/hour)'),
    y=alt.Y('Speed:Q', bin=alt.Bin(maxbins=50), title='Speed (mph)'),
    color=alt.Color('count()', title='Number of points', scale=alt.Scale(type='log')),
    tooltip=['count()']
)
